...
function sim.setCell( cellType, xworld, yworld)
  ...
  if map.isInsideGrid(xg, yg) then
    ...
    elseif (cellType == 'tunnel') then
      grid.pass = true
      grid.cell = TCell.newTunnel()
...
function pauseSim( time )
  for _,node in pairs(map.ants.array) do
    node.obj.pause( time )
  end
end

function unPauseSim( time )
  for _,node in pairs(map.ants.array) do
    node.obj.unPause( time )
  end
end
